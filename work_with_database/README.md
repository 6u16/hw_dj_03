...
- Создать БД createdb -U postgres netology_import_phones(указана в settings.py)
- Применить: python manage.py makemigrations и python manage.py migrate(при каждом изменении БД такие применения)
...
- Запустить перенос данных из csv-файла через терминал: python manage.py import_phones
